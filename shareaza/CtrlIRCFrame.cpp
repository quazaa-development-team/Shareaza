...
int CIRCFrame::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
	...
	if ( theApp.m_bRTL ) m_wndMainBar.ModifyStyleEx( 0, WS_EX_LAYOUTRTL, 0 );
...
void CIRCFrame::OnSkinChange()
{
	...
	m_wndPanel.m_boxChans.m_wndAddChannel.SetIcon( CoolInterface.ExtractIcon( ID_IRC_ADD ) );
	m_wndPanel.m_boxChans.m_wndRemoveChannel.SetIcon( CoolInterface.ExtractIcon( ID_IRC_REMOVE ) );
...
void CIRCFrame::DrawText(CDC* pDC, int nX, int nY, LPCTSTR pszText)
{
	...
	DWORD nOptions = ETO_CLIPPED | ( theApp.m_bRTL ? ETO_RTLREADING : 0 );
...
int CIRCFrame::AddTab(CString strTabName, int nKindOfTab)
{
	...
	int nNewTab = m_wndTab.InsertItem( TCIF_TEXT | ( theApp.m_bRTL ? TCIF_RTLREADING : 0 ), 
