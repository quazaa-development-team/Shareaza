...
{
	if ( pBuffer->m_nLength < sizeof( ED2K_TCP_HEADER ) )
		return NULL;
	...
	if ( pHeader->nProtocol != ED2K_PROTOCOL_EDONKEY
			...
			&& pHeader->nProtocol != ED2K_PROTOCOL_PACKED )
		return NULL;
	if ( pBuffer->m_nLength - sizeof( ED2K_TCP_HEADER ) + 1 < pHeader->nLength )
		return NULL;
	CEDPacket* pPacket = CEDPacket::New( pHeader );
	pBuffer->Remove( sizeof( ED2K_TCP_HEADER ) + pHeader->nLength - 1 );
	if ( pPacket->InflateOrRelease( nEdProtocol ) )
		return NULL;
	return pPacket;
}
