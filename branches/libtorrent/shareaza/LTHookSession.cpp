...
bit_impl::bit_impl() :
	session_(libt::fingerprint(LTHOOK_FINGERPRINT)),
	keepChecking_(false),
	...
	dht_on_(false)
{
	...
	{	libt::session_settings settings = session_.settings();
	    //TODO: Replace version info with Shareaza version info
		settings.user_agent = string("Shareaza ") + /*HALITE_VERSION_STRING*/;
		session_.set_settings(settings);
	}
	
	start_alert_handler();

	} LTHOOK_GENERIC_FN_EXCEPTION_CATCH(L"bit_impl::bit_impl()")
}
...
{	
	try
	{
	LTHOOK_DEV_MSG(L"Commence ~BitTorrent_impl"); 
...
//		write_vec_range(ofs, v6);
	...
	} LTHOOK_GENERIC_FN_EXCEPTION_CATCH(L"~BitTorrent_impl")
}
...
//			handler(*p_alert);
	...
	} LTHOOK_GENERIC_FN_EXCEPTION_CATCH(L"bit_impl::alert_handler()")
}
