...
BOOL CIRCPanel::Create(CWnd* pParentWnd)
{
	...
	return CTaskPanel::Create( WS_VISIBLE, rect, pParentWnd, IDC_IRC_PANEL );
}
...
int CIRCUsersBox::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
	...
	if ( theApp.m_bRTL ) m_wndUserList.ModifyStyleEx(WS_EX_LAYOUTRTL,0,0);
...
int CIRCChannelsBox::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
	...
	m_wndAddChannel.SetIcon( CoolInterface.ExtractIcon( ID_IRC_ADD ) );
	...
	m_wndRemoveChannel.SetIcon( CoolInterface.ExtractIcon( ID_IRC_REMOVE ) );
	m_wndAddChannel.SetHandCursor( TRUE );
